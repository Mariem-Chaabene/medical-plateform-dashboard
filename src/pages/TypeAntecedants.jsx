...
export default function TypeAntecedants() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/type-antecedants?per_page=500`, {
      headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
    })
      .then(async (res) => {
        const body = await res.json().catch(() => null);
        if (!res.ok) throw new Error(body?.message || "Erreur chargement types d’antécédents");
        return body;
      })
      .then((data) => {
  ...
  const actions = [
    { label: "Show", icon: iconShow, handler: (row) => navigate(`/type_antecedants/${row.id}`) },
    ...(role === "admin"
      ? [{ label: "Edit", icon: iconEdit, handler: (row) => navigate(`/type_antecedants/${row.id}/edit`) }]
      : []),
  ];

  return (
    <Layout>
      <h2>Types d’antécédents</h2>
        ...
        <div style={{ flexBasis: "20%", maxWidth: "20%", display: "flex", justifyContent: "flex-end" }}>
          {role === "admin" && (
            <button
              ...
              onClick={() => navigate("/type_antecedants/create")}
            >
              Ajouter
            </button>
          )}
      ...
      {role === "admin" && selected.length > 0 && (
        ...
        >
          ...
          <button
            ...
            onClick={async () => {
              if (!window.confirm("Supprimer les types d’antécédents sélectionnés ?")) return;
