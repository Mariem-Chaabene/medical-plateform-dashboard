...
export default function TypeAnalyses() {
  ...
  useEffect(() => {
    ...
    // on prend "assez" d'items pour paginer côté front (comme Patients/Roles)
    // tu peux augmenter si tu en as beaucoup
    const url = `http://127.0.0.1:8000/api/type-analyses?per_page=500`;
    ...
    fetch(url, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(body?.message || "Erreur chargement type analyses");
        return body;
      })
      .then((data) => {
        // paginate laravel => {data: [...]}
  ...
  const filtered = useMemo(() => {
    return items.filter((t) =>
      String(t.libelle || "").toLowerCase().includes(search.toLowerCase())
    );
  }, [items, search]);
  ...
  const actions = [
    { label: "Show", icon: iconShow, handler: (row) => navigate(`/type_analyses/${row.id}`) },
    ...(role === "admin"
      ? [{ label: "Edit", icon: iconEdit, handler: (row) => navigate(`/type_analyses/${row.id}/edit`) }]
      : []),
  ];
        ...
        <div style={{ flexBasis: "20%", maxWidth: "20%", display: "flex", justifyContent: "flex-end" }}>
          {role === "admin" && (
            <button
              ...
              onClick={() => navigate("/type_analyses/create")}
            >
              Ajouter
            </button>
          )}
      ...
      {role === "admin" && selected.length > 0 && (
        ...
        >
          ...
          <button
            ...
            onClick={async () => {
              if (!window.confirm("Supprimer les types d’analyses sélectionnés ?")) return;
              try {
                await Promise.all(
                  selected.map((id) =>
