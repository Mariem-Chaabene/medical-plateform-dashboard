...
export default function TypeExamenEdit() {
  const { id } = useParams();
  const { token } = useAuth();
  const navigate = useNavigate();

  const [values, setValues] = useState({ libelle: "" });
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/type-examens/${id}`, {
      ...
      .then((data) => setValues({  libelle: data.libelle || "" }))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/type-examens/${id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <Layout>
      <h2 style={{ marginBottom: 32 }}>Modifier Type d’examen</h2>
